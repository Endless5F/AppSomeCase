...
abstract class RecyclerAdapter : RecyclerView.Adapter<RecyclerAdapter.RecyclerViewHolder>() {
    ...
    companion object {
        const val HEADER_OFFSET = 100000
        const val FOOTER_OFFSET = 200000
    }
    ...
    abstract fun getInnerViewType(position: Int): Int

    protected abstract fun onCreateInnerViewHolder(
        parent: ViewGroup,
        viewType: Int
    ): RecyclerViewHolder
    ...
    protected fun onBindInnerViewHolder(
     ...
     */
    fun removeAllHeader() {
        if (!mHeaderViews.isNullOrEmpty()) {
            val headerCount = mHeaderViews.size
            mHeaderViews.clear()
