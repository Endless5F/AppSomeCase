...
class DebugClassVisitor(writer: ClassWriter) : ClassVisitor(Opcodes.ASM5, writer) {
    ...
    override fun visitMethod(
        ...
        return when(name) {
            METHOD_NAME_ON_CREATE -> {
                DebugMethodVisitor(api, methodVisitor, access, name, desc!!, className)
            }
            else -> methodVisitor
        }
    }
